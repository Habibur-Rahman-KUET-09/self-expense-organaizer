...
void main() {
  ...
  test(
    'upgrading a pre-existing (schema v2) database adds the Habit Tracker '
    'tables without touching existing budgets (backward compatibility)',
    () async {
      // Hand-build the v2 schema (post Min/Max rework, pre Habit Tracker)
      // and seed a budget row — simulating a device on the previous release.
      final raw = sqlite3.sqlite3.openInMemory();
      raw.execute('''
        CREATE TABLE categories (
          id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT,
      ...
      raw.execute('''
        CREATE TABLE budgets (
          ...
          threshold_percent REAL NOT NULL DEFAULT 90,
          threshold_base TEXT NOT NULL DEFAULT 'max',
          no_alert INTEGER NOT NULL DEFAULT 0
        );
      ''');
      raw.execute(
        "INSERT INTO categories (id, name, created_at) VALUES (1, 'Food', 0)",
      ...
      // The new tables exist and are usable — querying them would throw if
      // the additive migration hadn't created them.
      expect(await db.select(db.habitCategories).get(), isEmpty);
